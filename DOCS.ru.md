...
Проект может содержать файл конфигурации линтера. Используется файл `yfmlint.yaml` в корне проекта.
...
В конфиге `yfmlint.yaml` в секции `log-levels` вы можете переопределить уровень логгирования отдельно для каждого правила: `error`, `warn`, `disabled`.
