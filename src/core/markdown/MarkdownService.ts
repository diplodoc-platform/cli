...
import type {AdditionalInfo, HeadingInfo, IncludeInfo, Plugin} from './types';

import {join} from 'node:path';
import {uniq} from 'lodash';
import pmap from 'p-map';

import {Defer, SourceMap, bounded, langFromPath, memoize, normalizePath} from '~/core/utils';

import {getHooks, withHooks} from './hooks';
import {TransformMode, loader} from './loader';
import {Demand, lines} from './utils';
...
export class MarkdownService {
    ...
    private pathToAssets = new Demand<NormalizedPath[]>(this.load);
    ...
    async load(path: RelativePath, from: NormalizedPath[] = []) {
        ...
        const assets = this.pathToAssets.get(file) as NormalizedPath[];
        ...
        await pmap(assets, (asset) => getHooks(this).Asset.promise(asset, file));

        return content;
    }

    async dump(path: RelativePath, markdown: string) {
        const file = normalizePath(path);
    ...
    async deps(path: RelativePath, from: NormalizedPath[] = []) {
        ...
        return this.pathToDeps.onDemand(file, from, async (deps: IncludeInfo[]) => {
            const internals: IncludeInfo[][] = await pmap(deps, async ([dep, location]) => {
                const deps = await this.deps(dep, [...from, file]);
    ...
    async assets(path: RelativePath, from: NormalizedPath[] = []) {
        ...
        return this.pathToAssets.onDemand(file, from, async (assets: NormalizedPath[]) => {
            const internals: NormalizedPath[][] = await this.mapdeps(file, from, ([dep]) => {
                return this.assets(dep, [...from, file]);
            });
    ...
    async headings(path: RelativePath, from: NormalizedPath[] = []) {
        ...
        return this.pathToHeadings.onDemand(file, from, async (headings: HeadingInfo[]) => {
            const byLocation = ([, a]: HeadingInfo, [, b]: HeadingInfo) => a.start - b.start;
            const internals: HeadingInfo[][] = await this.mapdeps(
                file,
                from,
                async ([dep, location]) => {
                    const headings = await this.headings(dep, [...from, file]);
                    return headings.map(([dep]) => [dep, location]);
                },
            );
    ...
    private loaderContext(run: Run, path: NormalizedPath, raw: string, vars: Hash): LoaderContext {
        return {
            ...
            sourcemap: new SourceMap(lines(raw)),
            options: {
                rootInput: run.originalInput,
                allowHTML: run.config.allowHtml,
                needToSanitizeHtml: run.config.sanitizeHtml,
                useLegacyConditions: run.config.useLegacyConditions,
                supportGithubAnchors: Boolean(run.config.supportGithubAnchors),

                applyPresets: run.config.template.features.substitutions,
                resolveConditions: run.config.template.features.conditions,
