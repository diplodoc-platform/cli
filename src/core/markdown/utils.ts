 ...
 */
export function unescapeLiquid(escapedContent: string): string {
    return escapedContent.replace(/\(\({{/g, '{{').replace(/}}\)\)/g, '}}');
}
...
export class Demand<T> {
    ...
    async onDemand<R>(
        ...
        if (!this.scope.has(file)) {
            ...
            this.ondemand(file, from).catch(wait.reject).finally(wait.dispose);
        }
...
export class LineMap {
    ...
    location(from: number, to: number): Location {
        ...
        while (this.map.length < index) {
            if (from <= this.map[index]) {
        ...
        while (this.map.length < index) {
            if (to <= this.map[index]) {
                location.end = index + 1;
                break;
            }
