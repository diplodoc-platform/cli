...
function filterUsefullContent(filter: OpenApiIncluderParams['filter'], vars: YfmPreset) {
    const {endpoint: endpointFilter, tag: tagFilter} = filter || {};

    return (spec: Specification): Specification => {
        const filterTag = (tag: Tag) => tagFilter
            ? evalExp(tagFilter, {...tag, vars})
            : true;
        const filterEndpoint = (endpoint: Endpoint) => endpointFilter
            ? evalExp(endpointFilter, {...endpoint, vars})
            : true;
