...
>(
    ...
    const {config: {defaults = () => ({}), scope, strictScope} = {}, hooks = {}} = parts;
    ...
    return class BaseProgram implements IProgram<TArgs> {
        ...
        apply(program?: IProgram) {
            ...
            this.command.action((args: TArgs) => this._action(args));
        ...
        private async post() {
            ...
            if (error.count || (this.config.strict && warn.count)) {
                throw new HandledError('There is some build errors.');
            }
        }

        private async _action(args: TArgs) {
            await this.pre(args);
