...
function test(content: string, options: Options = {}, vars = {}, files = {}, copy = []) {
    return async () => {
        ...
        when(run.vars.load).calledWith('./toc.yaml' as RelativePath).thenResolve(vars);

        when(run.read)
            .calledWith(join(run.input, './toc.yaml' as RelativePath))
            .thenResolve(content);
...
describe('toc-loader', () => {
    ...
    // TODO: implement includers
    describe.skip('includers', () => {});
});
