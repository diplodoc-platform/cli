...
import type {IncluderOptions, Toc, TocItem} from '~/commands/build';
...
type Graph = {
    [prop: string]: NormalizedPath | Graph;
};
    ...
    apply(program: Build) {
        program.hooks.BeforeAnyRun.tap('GenericIncluder', (run) => {
            run.toc.hooks.Includer.for('generic').tapPromise(
                'GenericIncluder',
                async (toc: Toc, options: Options, path: RelativePath) => {
...
function fillToc(toc: Toc, graph: Graph, options: Options) {
    function item([key, value]: [string, Graph | NormalizedPath]): TocItem {
