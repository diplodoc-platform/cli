...
export function mergeSvg(
    run: Run,
    svgList: Map<string, string>,
    assets: AssetInfo[],
    ...
    return async function (scheduler, _entry): Promise<void> {
        ...
        const actor = async function (content: string, {image}: StepContext): Promise<string> {
            ...
            if (options.inline === false || !isEnable || !imgPath) {
                return content;
            }
            ...
            const [full, _title, oldSizes, _optionsString] = content
            ...
            if ((options as ImageOptions).inline === false) {
                return content;
            }
