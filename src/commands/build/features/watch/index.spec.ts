...
describe('Build watch feature', () => {
    ...
    function hasNode(store: 'toc' | 'vars' | 'entry', node: string, value = true) {
        expect(run(build)[store].graph.hasNode(node), `Graph ${store} has node ${node}`).toBe(
            value,
        );
    }

    function depends(store: 'toc' | 'vars' | 'entry', from: string, to: string, value = true) {
        const existsFrom = run(build)[store].graph.hasNode(from);

        if (existsFrom) {
            expect(
                run(build)[store].graph.dependenciesOf(from).includes(to),
    ...
    function nodeData(store: 'toc' | 'vars' | 'entry', node: string, value: object) {
        expect(run(build)[store].graph.hasNode(node), `Node ${node} exists in ${store} graph`).toBe(
            true,
        );
        expect(run(build)[store].graph.getNodeData(node)).toMatchObject(value);
    }
        ...
        it('should handle toc update includer source', async () => {
            ...
            getTocHooks(service)
                ...
                .tapPromise('Tests', async (_rawtoc, options) => {
                    ...
                    service.graph.addNode(input, {type: 'source', data: undefined});
                    service.graph.addDependency(options.from, input);
