...
// A link with a title stays in the skeleton at the fragment edge: the unit
// keeps its parts side by side, and the title is a unit of its own.
const EDGE_LINK = `- [Issue](${ISSUE} "Задача") исправлен.`;
...
describe('maskAddresses', () => {
    ...
    it('should send the parts of a link crossing the fragment edge as one placeholder', () => {
        const [, unit] = units(EDGE_LINK).texts;

        // `]`, `(`, the address, the title and `)` stand side by side.
        expect(unit.match(/<x /g)).toHaveLength(5);
...
describe('unmaskAddresses', () => {
    it('should give back the source for an echo of the masked fragment', () => {
        const markdown = [
            `- [Issue](${ISSUE} "Задача") исправлен, **важно**.`,
    ...
    it('should take the address of a link crossing the edge from the source', () => {
        const {texts, skeleton} = units(EDGE_LINK);
        const answer = 'Issue<x ctype="link_text_part_close" id="x-1"/> fixed.';

        expect(composed(skeleton, [texts[0], unmaskAddresses(texts[1], answer)])).toBe(
...
describe('renumberMemory', () => {
    ...
    it('should pair a run of link parts with another address as a whole', () => {
        const {
            ...
            EDGE_LINK,
            EDGE_LINK,
            `- [Задача](${ISSUE.replace('930', '931')} "Task") fixed.`,
            ['fixed', 'исправлена'],
        );

        expect(translation).toBe('Задача<x ctype="link_text_part_close" id="x-1"/> fixed.');
        expect(kept).toBe(true);
        expect(line).toBe(`- [Задача](${ISSUE} "Задача") исправлена.`);
    });
});

describe('keepsPlaceholders', () => {
    const markdown = [EDGE_LINK, `- Исправлено в [коммите](${COMMIT}) и [задаче](${ISSUE}).`].join(
        '\n',
    );
