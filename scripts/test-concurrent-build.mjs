...
function loadConfig(explicitPath) {
    const defaultPath = join(__dirname, 'test-concurrent-build.config.yaml');
    ...
    cfg.inputDir = expandPath(cfg.inputDir || '~/Projects/test/in/test_doc');
    cfg.outputBase = expandPath(cfg.outputBase || '~/Projects/test/concurrent-out');
    cfg.testRuns = Number.isFinite(cfg.testRuns) ? cfg.testRuns : 3;
    cfg.threadCounts = Array.isArray(cfg.threadCounts) ? cfg.threadCounts : [2];
    cfg.parallelTests = Number.isFinite(cfg.parallelTests) ? cfg.parallelTests : 1;
    cfg.verboseLogs = Boolean(cfg.verboseLogs);
    cfg.showAllMismatches = cfg.showAllMismatches !== false;
    cfg.saveReport = cfg.saveReport !== false;
    cfg.buildArgs = Array.isArray(cfg.buildArgs) ? cfg.buildArgs : [];
...
const TEST_RUNS = CONFIG.testRuns;
const THREAD_COUNTS = CONFIG.threadCounts;
const OUTPUT_BASE = CONFIG.outputBase;
const VERBOSE_LOGS = CONFIG.verboseLogs; // Set to true to see full build logs
const SHOW_ALL_MISMATCHES = CONFIG.showAllMismatches; // Set to false to show only first mismatch
const PARALLEL_TESTS = CONFIG.parallelTests; // Number of tests to run in parallel (set > 1 to max out CPU)
const BUILD_ARGS = CONFIG.buildArgs;
