    ...
    if (Array.isArray(obj)) {
        ...
        for (const key in obj) {
            if (newObj[key] && typeof newObj[key] === 'object' && !Array.isArray(newObj[key])) {
                ...
                newObj[key] = unionExtScheme(
                    obj[key],
                    Array.isArray(newObj[key]) ? extSchema[key] : {},
                );
            }
        }
        return newObj;
...
const extScheme = load(
    readFileSync(join(ROOT, 'schemas/page-constructor-schema-extend.yaml'), 'utf8'),
);
